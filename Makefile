...
libcfw.a: acl.o conntrack.o acl_hcl.o hcl.o
	$(AR) rcs $@ $^

.PHONY: fmt
fmt:
	clang-format -i acl.c acl.h conntrack.c conntrack.h acl_hcl.c acl_hcl.h test/*.c

.PHONY: test
test:
	$(CC) $(TEST_CFLAGS) acl.c test/acl_test.c -o test/acl_test && ./test/acl_test
	$(CC) $(TEST_CFLAGS) conntrack.c test/conntrack_test.c -o test/conntrack_test && ./test/conntrack_test
	$(CC) $(TEST_CFLAGS) acl.c acl_hcl.c $(HCL_DIR)/hcl.c test/acl_hcl_test.c -o test/acl_hcl_test && ./test/acl_hcl_test
...
cover:
	...
	$(CC) $(COVER_CFLAGS) acl.c acl_hcl.c $(HCL_DIR)/hcl.c test/acl_hcl_test.c -o test/acl_hcl_test
	LLVM_PROFILE_FILE=aclhcl.profraw ./test/acl_hcl_test >/dev/null
	$(LLVM_PROFDATA) merge -sparse acl.profraw -o acl.profdata
	$(LLVM_COV) report ./test/acl_test -instr-profile=acl.profdata acl.c
